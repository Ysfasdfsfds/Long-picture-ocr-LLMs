...
class LongImageOCR:
    ...
    def process_slices(self, slices_info: List[Dict]) -> List[Dict]:
        ...
        if is_feishu_screenshot:
            print("检测到飞书截图")
            marked_ocr_results = self._remark_content_with_deduplicated_data_feishu(deduplicated_ocr, deduplicated_avatars)
        else:
            print("执行微信 蓝信 钉钉 的标记")
            # 基于去重后的数据重新标记
            marked_ocr_results = self._remark_content_with_deduplicated_data_wechat(deduplicated_ocr, deduplicated_avatars)
    ...
    def _remark_content_with_deduplicated_data_wechat(self, deduplicated_ocr, deduplicated_avatars):
        """
        基于去重后的数据重新标记内容
        ...
        # 2. 基于头像位置标记昵称和内容
        self._mark_nickname_and_content_with_avatars_wechat(sorted_ocr, sorted_avatars)
    ...
    def _remark_content_with_deduplicated_data_feishu(self, deduplicated_ocr, deduplicated_avatars):
        """
        基于去重后的数据重新标记内容
        ...
        # 2. 标记系统消息
        self._mark_system_messages(sorted_ocr)
        
        # 3. 基于头像位置标记昵称和内容
        self._mark_nickname_and_content_with_avatars_feishu(sorted_ocr, sorted_avatars)
    ...
    def _mark_system_messages(self, ocr_results):
        """
        标记系统消息（如：欢迎..加入...等模式）
