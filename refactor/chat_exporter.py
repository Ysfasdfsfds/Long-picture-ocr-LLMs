...
def export_structured_chat_messages(marked_ocr_results_original: List[Dict], 
    ...
    Args:
        marked_ocr_results_original: 标记后的OCR结果列表
        output_json_dir: 输出JSON目录
    ...
    if output_path is None:
        output_path = output_json_dir / "structured_chat_messages.json"
    
    if not marked_ocr_results_original:
        print("没有标记后的OCR结果可整理")
        return ""
    
    # 提取所有标记后的文本
    marked_texts = [item.get('text', '') for item in marked_ocr_results_original]
    
    # 整理成结构化聊天消息
    structured_messages = organize_chat_messages(marked_texts)
...
def organize_chat_messages(marked_texts: List[str]) -> List[Dict]:
    """
    将标记后的文本组织成结构化聊天消息
    ...
    """
    # 首先分析所有昵称信息
    nickname_analysis = analyze_nicknames(marked_texts)
    ...
    while i < len(marked_texts):
        ...
        if "(昵称)" in text:
            ...
            if content_parts:
                ...
                # 没有内容，检查是否为群聊名称
                if is_group_name(nickname, i, nickname_analysis):
                    messages.append({
                        "type": "group_name",
                        "群聊名称": nickname
                    })
...
def analyze_nicknames(marked_texts: List[str]) -> Dict:
    """
    分析所有昵称的出现情况
...
def is_group_name(nickname: str, position: int, nickname_analysis: Dict) -> bool:
    """
    判断是否为群聊名称
...
def is_time_content(text: str) -> bool:
    """
    检查文本是否为时间内容
