...
def deduplicate_results(ocr_results: List[Dict], avatar_positions: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
    """
    对OCR结果和头像位置进行去重
    ...
    # 转换numpy类型为Python原生类型，以便于JSON序列化
    deduplicated_ocr = convert_numpy_types(deduplicated_ocr)
    deduplicated_avatars = convert_numpy_types(deduplicated_avatars)
