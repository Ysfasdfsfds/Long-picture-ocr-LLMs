...
class OCRExperimentRunner:
    ...
    def __init__(self, base_config_path: str = "default_rapidocr.yaml", 
        ...
        for ocr_config in base_ocr_configs:
            for slice_height, slice_width in slice_sizes:
    ...
    def _create_experiment_config(self, limit_type: str, limit_side_len: int, 
                                slice_height: int, slice_width: int) -> tuple:
        """
        创建实验配置文件
        ...
        Returns:
            (rapidocr_config_path, slice_config_path): 配置文件路径元组
        """
        # 复制基础配置
        ...
        # 创建实验目录
        exp_name = f"{limit_type}_{limit_side_len}_slice_{slice_height}x{slice_width}"
        exp_dir = self.experiments_dir / exp_name
        exp_dir.mkdir(parents=True, exist_ok=True)
        
        # 保存RapidOCR配置文件
        rapidocr_config_path = exp_dir / "config.yaml"
        with open(rapidocr_config_path, 'w', encoding='utf-8') as f:
        ...
        slice_config = {
            ...
            "slice_width": slice_width,
            "overlap": 200,  # 默认y方向重叠
            "x_overlap": 100,  # 默认x方向重叠
            "experiment_name": exp_name
        }
        
        slice_config_path = exp_dir / "slice_config.json"
        with open(slice_config_path, 'w', encoding='utf-8') as f:
            json.dump(slice_config, f, indent=2)
        
        return str(rapidocr_config_path), str(slice_config_path)
    ...
    def _run_single_experiment(self, limit_type: str, limit_side_len: int, 
        ...
        logger.info(f"  OCR参数: limit_type={limit_type}, limit_side_len={limit_side_len}")
        logger.info(f"  切片参数: slice_height={slice_height}, slice_width={slice_width}")
        
        # 创建配置文件
        rapidocr_config_path, slice_config_path = self._create_experiment_config(
            limit_type, limit_side_len, slice_height, slice_width)
        ...
        try:
            ...
            # 初始化OCR处理器并应用切片配置
            processor = LongImageOCR(config_path=rapidocr_config_path)
            
            # 动态设置切片参数
            processor.config.image.slice_height = slice_height
            processor.config.image.slice_width = slice_width
            processor.image_slicer.slice_height = slice_height
            processor.image_slicer.slice_width = slice_width
    ...
    def _generate_experiment_report(self, result: Dict, exp_dir: Path):
        ...
        report_lines.append(f"- limit_type: {params.get('limit_type')}")
        report_lines.append(f"- limit_side_len: {params.get('limit_side_len')}")
        report_lines.append(f"- slice_height: {params.get('slice_height')}")
        report_lines.append(f"- slice_width: {params.get('slice_width')}")
        report_lines.append("")
    ...
    def run_all_experiments(self, image_path: str):
        ...
        for i, config in enumerate(self.experiment_configs, 1):
            ...
            exp_name = f"{config['limit_type']}_{config['limit_side_len']}_slice_{config['slice_height']}x{config['slice_width']}"
    ...
    def _generate_detailed_comparison_report(self):
        ...
        for exp_name, data in experiment_data.items():
            # 提取切片尺寸信息
            if 'slice_' in exp_name:
                slice_part = exp_name.split('slice_')[1]
                if 'x' in slice_part:
                    slice_size = slice_part.split('x')[0]
                    if slice_size not in slice_performance:
                        slice_performance[slice_size] = []
                    slice_performance[slice_size].append(data['total_experiment_time'])
        
        if slice_performance:
            report_lines.append("### 2. 切片尺寸影响分析")
            slice_avg_times = {}
            for size, times in slice_performance.items():
                avg_time = sum(times) / len(times)
                slice_avg_times[size] = avg_time
                report_lines.append(f"- **{size}x{size}切片**: 平均 {avg_time:.2f}秒 ({len(times)}个配置)")
            
            # 找出最优切片尺寸
            best_slice_size = min(slice_avg_times, key=slice_avg_times.get)
            worst_slice_size = max(slice_avg_times, key=slice_avg_times.get)
            improvement = ((slice_avg_times[worst_slice_size] - slice_avg_times[best_slice_size]) / 
                          slice_avg_times[worst_slice_size]) * 100
            
            report_lines.append(f"- **最优切片尺寸**: {best_slice_size}x{best_slice_size} ({slice_avg_times[best_slice_size]:.2f}s)")
            report_lines.append(f"- **最差切片尺寸**: {worst_slice_size}x{worst_slice_size} ({slice_avg_times[worst_slice_size]:.2f}s)")
            report_lines.append(f"- **性能差异**: 最优比最差快 {improvement:.1f}%")
            report_lines.append("")
        ...
        # 切片尺寸建议
        if 'slice_' in best_name:
            best_slice_part = best_name.split('slice_')[1]
            if 'x' in best_slice_part:
                best_slice_size = best_slice_part.split('x')[0]
                report_lines.append(f"5. **切片尺寸选择**: 推荐使用 `{best_slice_size}x{best_slice_size}`，性能最优")
        
        # 组合建议
        if slice_performance:
            report_lines.append("6. **参数组合建议**: OCR参数和切片尺寸存在相互影响，建议综合测试选择最优组合")
...
def auto_commit_changes():
    ...
    try:
        ...
        if result.stdout.strip():
            ...
            # 提交变更
            commit_message = "feat: 添加切片尺寸参数实验支持\n\n- 支持(400-1200)x(400-1200)切片尺寸测试\n- 与现有OCR参数一一组合(108种配置)\n- 扩展实验报告分析切片尺寸影响\n- 自动提交实验结果"
