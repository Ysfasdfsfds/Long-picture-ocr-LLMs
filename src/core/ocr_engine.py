        ...
        try:
            ...
            if save_visualization:
                try:
                    ...
                    logger.warning(f"切片 {slice_info.slice_index} 可视化保存失败: {viz_error}")
                    # 可视化失败不影响OCR结果处理
            ...
            logger.error(f"OCR处理失败 - 切片 {slice_info.slice_index}: {e}", exc_info=True)
            logger.info(f"切片 {slice_info.slice_index} 跳过OCR识别，返回空OCR结果")
            # result 已经是空的 SliceOCRResult，直接返回
