...
class JsonExporter:
    ...
    def export_chat_messages(self, session: ChatSession, 
        ...
        try:
            ...
            # 记录错误详情
            logger.error(f"JSON文件写入失败: {output_path}, 错误: {e}", exc_info=True)
            
            # 尝试备用路径
            try:
                temp_dir = Path(tempfile.gettempdir())
                backup_path = temp_dir / f"backup_{output_file}"
                
                with open(backup_path, 'w', encoding='utf-8') as f:
                    json.dump(export_data, f, ensure_ascii=False, indent=2)
                
                logger.warning(f"已使用备用路径保存: {backup_path}")
                return str(backup_path)
                
            except Exception as backup_error:
                logger.error(f"备用路径写入也失败: {backup_error}", exc_info=True)
                # 返回空字符串但不中断程序
                return ""
    ...
    def export_marked_ocr_results(self, marked_items: List[OCRItem],
        ...
        try:
            ...
            # 记录错误详情
            logger.error(f"标记OCR结果文件写入失败: {output_path}, 错误: {e}", exc_info=True)
    ...
    def export_summary_data(self, ocr_items: List[OCRItem], 
        ...
        try:
            ...
            # 记录错误详情
            logger.error(f"汇总数据文件写入失败: {output_path}, 错误: {e}", exc_info=True)
