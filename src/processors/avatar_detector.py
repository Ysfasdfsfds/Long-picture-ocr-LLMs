...
class AvatarDetector:
    ...
    def detect_avatars(self, slice_info: SliceInfo, x_crop: Optional[int] = None) -> List[AvatarItem]:
        ...
        try:
            ...
            logger.error(f"头像检测失败 - 切片 {slice_info.slice_index}: {e}", exc_info=True)
            logger.info(f"切片 {slice_info.slice_index} 跳过头像检测，返回空头像列表")
            return []  # 返回空列表，允许OCR处理继续进行
    ...
    def _get_slices_to_process(self, slice_infos: List[SliceInfo]) -> List[SliceInfo]:
        ...
        if total_slices == 1:
            ...
            return slice_infos[:1]
        else:
            return slice_infos[1:-1]
