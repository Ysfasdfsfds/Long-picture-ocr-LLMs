...
class Deduplicator:
    ...
    def _deduplicate_ocr(self, ocr_items: List[OCRItem]) -> List[OCRItem]:
        ...
        for current_item in ocr_items:
            ...
            for i, existing_item in enumerate(deduplicated):
                ...
                text_similarity = self._calculate_text_similarity(current_item.text, existing_item.text)
                y_distance = self._calculate_y_distance(current_item.box, existing_item.box)
                
                # 判断是否为重复：IOU高 OR (文本相同且Y坐标接近)
                is_iou_duplicate = iou > self.ocr_iou_threshold
                is_text_duplicate = (text_similarity > 0.85 and y_distance < 50)  # 文本85%相似且Y距离<50像素
                
                if is_iou_duplicate or is_text_duplicate:
                    is_duplicate = True
                    # 保留置信度更高的结果
                    if current_item.score > existing_item.score:
                        deduplicated[i] = current_item
                        logger.debug(f"替换重复OCR (IoU={iou:.3f}, 文本相似度={text_similarity:.3f}, Y距离={y_distance:.1f}): "
                                   f"'{existing_item.text}' -> '{current_item.text}'")
                    else:
                        logger.debug(f"跳过重复OCR (IoU={iou:.3f}, 文本相似度={text_similarity:.3f}, Y距离={y_distance:.1f}): '{current_item.text}'")
                    break
    ...
    def _calculate_y_distance(self, box1: Any, box2: Any) -> float:
        """
        计算两个框的Y轴距离（中心点距离）
        ...
        try:
            ...
            # 计算中心Y坐标
            center_y1 = (b1['min_y'] + b1['max_y']) / 2
            center_y2 = (b2['min_y'] + b2['max_y']) / 2
            
            return abs(center_y1 - center_y2)
            
        except Exception as e:
            logger.error(f"计算Y距离时出错: {e}")
            return float('inf')